...
const InfoChange = () => {
  ...
  const isInViewContainer = useInView(sectionContainerRef);
  const isInView1 = useInView(section1Ref, { amount: 1 });
  const isInView2 = useInView(section2Ref, { amount: 1 });
  const isInView3 = useInView(section3Ref, { amount: 1 });
  const isInView4 = useInView(section4Ref, { amount: 1 });
...
const InfoSection = forwardRef<HTMLDivElement, { title: string; description: string; bgColor: string }>(
  ({ title, description, bgColor }, ref) => {
    return (
      <section className={`h-screen py-[100px] px-[80px] text-center`}>
        <h3 ref={ref} className="text-[110px] font-semibold mb-[30px]">
          {title}
        </h3>
